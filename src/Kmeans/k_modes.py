...
def _update_centroids(X, centroids, n_clusters, n_attrs, labels):
    _cluster_attr_fq = [[defaultdict(int) for _ in range(n_attrs)]
                        for _ in range(n_clusters)]
    for ipoint, point in enumerate(X):
        for iattr, attr_value in enumerate(point):
            cl = labels[ipoint]
            _cluster_attr_fq[cl][iattr][attr_value] += 1
    for ipoint, point in enumerate(centroids):
        for iattr, attr_value in enumerate(point):
            centroids[ipoint, iattr] = get_max_value_key(_cluster_attr_fq[ipoint][iattr])
...
def perform_empty_clusters(X, n_clusters, random_state,
    ...
    return labels, membership_matrix, centroids, moves, costs
...
def _bkmodes(X, n_clusters, n_points, n_attrs, max_iter, fdissimilarity, init, init_no,
    ...
    centroids, _cluster_attrs = _update_centroids(X, centroids, n_clusters, n_attrs, labels)
    ...
    while itr < max_iter and not converged:
        ...
        labels, membership_matrix, centroids, _moves, _ncost = perform_empty_clusters(X, n_clusters, random_state,
                                                                                      centroids, membership_matrix,
                                                                                      labels, fdissimilarity)
        perform_time_step = time.time()
        moves += moves
        ncost -= _ncost
        centroids, _cluster_attr = _update_centroids(X, centroids, n_clusters, n_attrs, labels)
...
if __name__ == '__main__':
    ...
    n_points, n_attrs = data2.shape
    _bkmodes(data2, 2, n_points, n_attrs, 10,
             jaccard_dissimilarity, modes2, 2, True, None,
             'timing', '/home/heydar/me/BSC/FinalPorject/lsh/src/benchmark/local/Result/t1')
