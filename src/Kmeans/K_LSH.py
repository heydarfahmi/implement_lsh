...
def jaccard_dissimilarity(a: set, b: set, **_):
    bsize = len(b)
    intersection_list=[(len(aa)+bsize , len(b.intersection(aa))) for aa in a]
    return np.array([1-(j[1]/(j[0]-j[1])) for j in intersection_list])
...
def _lshkmodes(X, n_clusters,Freq,lsh, n_points, n_attrs, max_iter, fdissimilarity, init, init_no,
    ...
    if isinstance(init, str) and init.lower() == 'lsh++':
        ...
        centroids = np.array(X,dtype=object)[seeds]
        #TODO
        # centroids=[set(c for c in centroids]
    ...
    while itr < max_iter and not converged:
        ...
        converged = ((moves == 0) or (ncost >= cost)) and (itr>1)
...
def ck_modes(X, n_clusters, max_iter, fdissimilarity, init, n_init, verbose, random_state, save, save_dir
             ):
    """k-modes algorithm"""
    random_state = get_random_state(random_state)
...
class CoordinateKmodes:
    ...
    def fit(self, X, **kwargs):
        ...
        (self._enc_cluster_centroids, self._enc_map, self.labels_, self.cost_,
         self.n_iter_, self.epoch_costs_) = ck_modes(
            X,
            self.n_clusters,
            self.max_iter,
            self.dissimilarity,
